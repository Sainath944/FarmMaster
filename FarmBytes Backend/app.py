...
#     # Make a prediction using the input array
#     prediction = fertilizer_model.predict([int(i) for i in x])
...
from typing import List, Union
from fastapi.middleware.cors import CORSMiddleware
from models import get_crop_model, get_fertilizer_model, get_input, get_crop_model_location
...
crop_model = get_crop_model()
fertilizer_model = get_fertilizer_model()
location_model = get_crop_model_location()
...
def array_endpoint(request: Request, input: InputArray):
    ...
    print("Making Crop Prediction...")
    # Convert input array to numeric values
    numeric_array = [float(i) for i in input.array]
    # Make a prediction using the input array
    prediction = crop_model.predict([numeric_array])
    prediction = prediction.tolist()[0]
...
def array_endpoint(request: Request, input: InputArray):
    ...
    print("Making Fertilizer Prediction...")
    # Convert input array to numeric values
    numeric_array = [int(i) for i in x]
    # Make a prediction using the input array
    prediction = fertilizer_model.predict([numeric_array])
    prediction = prediction[0]
...
def array_endpoint(request: Request, input: InputArray):
    ...
    numeric_array = [float(i) for i in input.array]
    # Make a prediction using the input array
    prediction = location_model.predict([numeric_array])
    print()
    prediction = prediction.tolist()[0]
